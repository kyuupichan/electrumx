...
def test_sha256():
    b_string = b'][\t\xf6\xdc\xb2\xd5:_\xff\xc6\x0cJ\xc0\xd5_\xab\xdf' \
               b'U`i\xd6c\x15E\xf4*\xa6\xe3P\x0f.'
    assert lib_hash.sha256(b'sha256') == b_string
    with pytest.raises(TypeError):
        lib_hash.sha256('sha256')


def ripemd160(x):
    b_string = b'\x903\x91\xa1\xc0I\x9e\xc8\xdf\xb5\x1aSK\xa5VW\xf9|W\xd5'
    assert lib_hash.ripemd160(b'ripemd160') == b_string
    with pytest.raises(TypeError):
        lib_hash.ripemd160('ripemd160')


def test_double_sha256():
    b_string = b'ksn\x8e\xb7\xb9\x0f\xf6\xd9\xad\x88\xd9#\xa1\xbcU(j1' \
               b'Bx\xce\xd5;s\xectL\xe7\xc5\xb4\x00'
    assert lib_hash.double_sha256(b'double_sha256') == b_string


def test_hmac_sha512():
    b_string = b"\xe4w8M|\xa2)\xdd\x14&\xe6Kc\xeb\xf2\xd3n\xbdm~f\x9a" \
               b"g5BNr\xeal\x01\xd3\xf8\xb5n\xb3\x9c6\xd8#/T" \
               b"'\x99\x9b\x8d\x1a?\x9c\xd1\x12\x8f\xc6\x9fMu\xb44" \
               b"!h\x10\xfa6~\x98"
    assert lib_hash.hmac_sha512(b'key', b'message') == b_string


def test_hash160():
    b_string = b'\xb3\x96\x94\xfc\x978R\xa7)XqY\xbb\xdc\xeb\xac\xa7%\xb8$'
    assert lib_hash.hash160(b'hash_160') == b_string
...
def test_Base58_decode():
    ...
    b_string = b'\x00\x01\x11\xd3\x8e_\xc9\x07\x1f\xfc\xd2\x0bJv<\xc9\xaeO%' \
               b'+\xb4\xe4\x8f\xd6j\x83^%*\xda\x93\xffH\rm\xd4=\xc6*d\x11U\xa5'
    assert lib_hash.Base58.decode('123456789ABCDEFGHJKLMNPQRSTUVWXYZ'
                                  'abcdefghijkmnopqrstuvwxyz') == b_string
